...
function getUserSelectors(): string[] {
  const configured = (() => {
    try {
      return (
        localStorage.getItem('claudeTimelineUserTurnSelector') ||
        localStorage.getItem('claudeTimelineUserTurnSelectorAuto') ||
        ''
      );
    } catch {
      return '';
...
function extractConversationIdFromUrl(): string | null {
  // Claude `/chat/<uuid>`, plus legacy ChatGPT `/c/<uuid>`.
  const chatMatch = window.location.pathname.match(/\/(?:chat|c)\/([^/?#]+)/);
...
function getConversationTitleForExport(): string {
  // Strategy 1: Get from active conversation in Claude-Voyager Folder UI (most accurate)
  try {
...
async function executeExportSequence(
  ...
  if (state.attempt > 25) {
    console.warn('[Claude-Voyager] Export aborted: too many attempts.');
  ...
  // 1. Find Top Node
  if (state.attempt > 0) {
    console.log('[Claude-Voyager] Resuming export... waiting for content load.');
    const userSelectors = getUserSelectors();
  ...
  if (!topNode) {
    console.log('[Claude-Voyager] No top node found, proceeding to export directly.');
    sessionStorage.removeItem(SESSION_KEY_PENDING_EXPORT);
  ...
  console.log(`[Claude-Voyager] Simulating click on top node (Attempt ${state.attempt + 1})...`);
  ...
  try {
    ...
    console.error('[Claude-Voyager] Failed to click top node:', e);
  }
  ...
  if (changed) {
    console.log('[Claude-Voyager] History expanded (soft refresh). Clicking top node again...');
  ...
  console.log('[Claude-Voyager] No refresh or update detected. Exporting...');
  sessionStorage.removeItem(SESSION_KEY_PENDING_EXPORT);
  await performFinalExport(
    format,
    dict,
...
async function performFinalExport(
  ...
  exportBtn.addEventListener('click', async (ev) => {
    ...
    try {
      ...
      console.error('[Claude-Voyager] Export error:', err);
...
async function checkPendingExport() {
  try {
    ...
    console.log('[Claude-Voyager] Resuming pending export sequence...');
    ...
    console.error('[Claude-Voyager] Failed to resume pending export:', e);
    sessionStorage.removeItem(SESSION_KEY_PENDING_EXPORT);
  }
}
...
async function handleResponseCopyImageClick(
  ...
  try {
    ...
    console.error('[Claude-Voyager] Failed to copy response image:', error);
...
export async function startExportButton(): Promise<void> {
  ...
  btn.addEventListener('click', (ev) => {
    ...
    try {
      ...
      try {
        console.error('Claude-Voyager export failed', err);
      } catch {}
    }
  });
  ...
  const reinjectExportButtonIfNeeded = () => {
    ...
    reinjectTimer = setTimeout(() => {
      ...
      try {
        ...
        try {
          console.debug('[Claude-Voyager] Export button re-injection failed:', e);
        } catch {}
      }
    }, 800);
  };
