...
const STORAGE_KEY = 'cvFolderData';
const IS_DEBUG = false; // Set to true to enable debug logging

/**
 * Read the current Claude conversation UUID from the URL.  TimelineV2
 * stores starred messages with `conversationId = <uuid>` (raw, no prefix),
 * so the favorites sidebar's currentId comparison must use the same shape
 * — otherwise every same-conv jump would falsely prompt the cross-conv
 * confirmation dialog.
 */
function getCurrentClaudeConvId(): string | null {
  const m = /\/chat\/([a-f0-9-]{36})/i.exec(window.location.pathname);
  return m?.[1] ?? null;
}

/**
 * In-page jump: ask TimelineV2 (via custom event) to scroll the current
 * conversation to the given turn.  We also pass the stored `content`
 * preview so TimelineV2 can fall back to a text-content match when the
 * stored turnId no longer matches a live node (Claude's
 * `data-test-render-count` increments across reloads, which made any
 * star created in a previous session resolve to "no match").
 */
function jumpStarredInPage(message: StarredMessage): void {
...
/**
 * Cross-conversation jump: warn the user that we're leaving the current
 * chat, then stash the target turn id + text content in sessionStorage
 * and navigate away.  The destination page reads the stash on startup
 * and tries to scroll once the dots are mounted; the content acts as a
 * fallback resolver when the stored id resolves to nothing on the new
 * render (see TimelineV2.jumpToTurn).
 */
async function jumpStarredCrossConversation(message: StarredMessage): Promise<void> {
  ...
  try {
    ...
    console.warn('[Claude-Voyager] cross-conv jump failed', error);
  }
}
...
export const SESSION_BACKUP_KEY = 'cvFolderBackup';
export const SESSION_BACKUP_TIMESTAMP_KEY = 'cvFolderBackupTimestamp';
...
export class FolderManager {
  ...
  private isDebugEnabled(): boolean {
    try {
      ...
      return IS_DEBUG || localStorage.getItem('cvFolderDebug') === '1';
    } catch {
  ...
  private async openFloatingPanel(): Promise<void> {
    ...
    this.floatingPanelHandle = mountFloatingPanel({
      ...
      mountFavorites: (host) => {
        // Mount the starred-messages sidebar into the host (a popover the
        // floating panel owns).  We pass Claude-aware callbacks: in-page
        // jumps go through a custom event TimelineV2 listens for, and
        // cross-conv jumps show a confirmation dialog before navigating.
        const sidebar = mountFavoritesSidebar({
          getCurrentConversationId: () => getCurrentClaudeConvId(),
          onJumpInPage: (msg) => jumpStarredInPage(msg),
          onJumpCrossConversation: async (msg) => {
            await jumpStarredCrossConversation(msg);
          },
        });
        host.appendChild(sidebar.element);
        // Subscribe to starred-change events so an open popover stays in
        // sync without us having to re-open it.  Silent background
        // refresh — the popover doesn't auto-pop-up, just updates content.
        const onStarChanged = () => void sidebar.refresh();
        const unsubAdded = eventBus.on('starred:added', onStarChanged);
        const unsubRemoved = eventBus.on('starred:removed', onStarChanged);
        return {
  ...
  private async performMigration(): Promise<void> {
    try {
      const result = await chrome.storage.local.get('cvSyncMode');
