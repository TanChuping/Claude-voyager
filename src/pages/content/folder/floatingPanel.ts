...
export type MountArgs = {
  ...
  /**
   * Optional favorites sidebar adapter — when provided, the panel header
   * shows a ⭐ button that toggles a popover containing the sidebar.  The
   * adapter is called once with the popover host element (the caller mounts
   * the sidebar into it) and returns hooks for refresh / cleanup.
   */
  mountFavorites?: (host: HTMLElement) => {
...
export function mountFloatingPanel({
  ...
  // ⭐ favorites toggle — opens a popover containing the starred-messages
  // sidebar.  Hidden when no `mountFavorites` callback is supplied.
  let favoritesBtn: HTMLButtonElement | null = null;
  let favoritesPopover: HTMLDivElement | null = null;
  let favoritesAdapter: { refresh: () => void | Promise<void>; destroy: () => void } | null = null;
  let favoritesOpen = false;
  if (mountFavorites) {
    favoritesBtn = createIconButton('favorites', 'favoritesButtonTitle', '★', (e) => {
      e.stopPropagation();
      if (favoritesOpen) closeFavorites();
      else openFavorites();
    });
    favoritesBtn.classList.add(`${FLOATING_PANEL_CLASS}__icon-button--favorites`);

    favoritesPopover = document.createElement('div');
    favoritesPopover.className = `${FLOATING_PANEL_CLASS}__favorites-popover`;
    favoritesPopover.setAttribute('role', 'dialog');
    favoritesPopover.setAttribute('aria-modal', 'false');
    favoritesPopover.style.cssText = [
      'position: fixed',
      'display: none',
      'z-index: 2147483647',
      'background: #FBF7EF',
    ...
    favoritesAdapter = mountFavorites(favoritesPopover);
  }
  ...
  if (favoritesBtn) headerActions.appendChild(favoritesBtn);
  headerActions.appendChild(createBtn);
  headerActions.appendChild(closeBtn);
  header.appendChild(headerActions);

  function positionFavoritesPopover(): void {
    if (!favoritesPopover || !favoritesBtn) return;
    const r = favoritesBtn.getBoundingClientRect();
    const popH = favoritesPopover.offsetHeight || 360;
    const popW = favoritesPopover.offsetWidth || 320;
    const vw = window.innerWidth;
    const vh = window.innerHeight;
    const pad = 8;
    // Place below the button by default, flip up if no room.
    let top = Math.round(r.bottom + 6);
    if (top + popH + pad > vh) top = Math.max(pad, Math.round(r.top - popH - 6));
    let left = Math.round(r.left + r.width / 2 - popW / 2);
    if (left + popW + pad > vw) left = Math.max(pad, vw - popW - pad);
    if (left < pad) left = pad;
    favoritesPopover.style.top = `${top}px`;
    favoritesPopover.style.left = `${left}px`;
  }
  function openFavorites(): void {
    if (!favoritesPopover || !favoritesAdapter || !favoritesBtn) return;
    favoritesOpen = true;
    favoritesPopover.style.display = 'block';
    favoritesBtn.classList.add(`${FLOATING_PANEL_CLASS}__icon-button--active`);
    void favoritesAdapter.refresh();
    requestAnimationFrame(positionFavoritesPopover);
    document.addEventListener('pointerdown', onFavoritesOutsideClick, true);
    document.addEventListener('keydown', onFavoritesKey, true);
    window.addEventListener('resize', positionFavoritesPopover);
  }
  function closeFavorites(): void {
    if (!favoritesPopover || !favoritesBtn) return;
    favoritesOpen = false;
    favoritesPopover.style.display = 'none';
    favoritesBtn.classList.remove(`${FLOATING_PANEL_CLASS}__icon-button--active`);
    document.removeEventListener('pointerdown', onFavoritesOutsideClick, true);
    document.removeEventListener('keydown', onFavoritesKey, true);
    window.removeEventListener('resize', positionFavoritesPopover);
  }
  function onFavoritesOutsideClick(e: PointerEvent): void {
    if (!favoritesOpen || !favoritesPopover || !favoritesBtn) return;
    const t = e.target as Node | null;
    if (!t) return;
    if (favoritesPopover.contains(t)) return;
    if (favoritesBtn.contains(t)) return;
    closeFavorites();
  }
  function onFavoritesKey(e: KeyboardEvent): void {
  ...
  const destroy = () => {
    ...
    inlineFormCleanup?.();
    inlineFormCleanup = null;
    // Tear down the favorites popover if it was mounted.
    if (favoritesOpen) closeFavorites();
    favoritesAdapter?.destroy();
    favoritesAdapter = null;
    favoritesPopover?.remove();
    favoritesPopover = null;
    panel.remove();
  };
