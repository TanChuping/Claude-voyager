...
/**
 * Read the current Claude conversation UUID from the URL — same raw
 * `<uuid>` format that TimelineV2 stores in `StarredMessage.conversationId`.
 *
 * Previously this returned the prefixed `cv:conv:<uuid>` (legacy
 * GPT-Voyager identity format), which never matched what TimelineV2
 * actually wrote — so every same-conv jump fell into the cross-conv
 * branch and pointlessly prompted the warning dialog.  Match the raw
 * UUID instead.  Returns null on the new-chat page.
 */
function getCurrentChatGptConversationId(): string | null {
  const m = /\/chat\/([a-f0-9-]{36})/i.exec(window.location.pathname);
  return m?.[1] ?? null;
}

/**
 * Same-conversation jump.  TimelineV2 listens for `cv:jump-to-turn` and
 * treats it like a programmatic dot click — and it has a content-based
 * fallback for stars whose turnId no longer resolves to a live node (see
 * `resolveTurnIdToIdx`).  So we plumb the stored content preview too.
 *
 * Falls back to the old `#gv-turn-` hash mechanism if the dispatch fails
 * — that's the path the legacy GPT-Voyager timeline manager listened
 * for, kept here so we don't strand anyone on an older code path.
 */
function jumpToStarredInPage(message: StarredMessage): void {
  const turnId = message.turnId;
  if (!turnId) return;
  try {
    document.dispatchEvent(
      new CustomEvent('cv:jump-to-turn', {
        detail: { turnId, content: message.content ?? '' },
      }),
    );
    return;
  } catch {
...
/**
 * Confirm with the user before leaving the current conversation, then
 * stash the target turnId + text content in sessionStorage and navigate
 * away.  TimelineV2 on the destination page reads the stash on startup
 * and scrolls once dots are mounted — the content acts as a fallback
 * resolver when the stored turnId no longer resolves to a live node
 * (Claude's data-test-render-count increments across reloads, so stars
 * created in a previous session almost always fail an exact id lookup).
 *
 * The OLD implementation here used `<convUrl>#gv-turn-<id>` hash
 * navigation, which TimelineV2 doesn't listen for — so cross-conv jumps
 * from the prompt-manager favorites popover used to land on the
 * destination's default scroll position (almost always the bottom) and
 * leave the user wondering why "jump" did nothing useful.
 */
async function jumpToStarredCrossConversation(message: StarredMessage): Promise<void> {
  ...
  if (!confirmed) return;
  if (!message.conversationId || !message.turnId) return;
  try {
    const stash = JSON.stringify({
      turnId: message.turnId,
      content: message.content ?? '',
    });
    sessionStorage.setItem(`cv-pending-jump:${message.conversationId}`, stash);
    // Prefer the stored conversationUrl when present; fall back to
    // /chat/<uuid> for legacy entries written before we recorded URLs.
    const targetUrl =
      message.conversationUrl && /\/chat\//i.test(message.conversationUrl)
...
export async function startPromptManager(): Promise<{ destroy: () => void }> {
  ...
  try {
    ...
    titleText.textContent = 'Claude-Voyager';
    ...
    const favoritesSidebar = mountFavoritesSidebar({
      ...
      onJumpInPage: (message) => jumpToStarredInPage(message),
    ...
    // Whenever the user stars/unstars a message anywhere on the page
    // (timeline dot, preview panel, etc.) we silently refresh the
    // favorites list so it's up-to-date next time the user opens it.
    //
    // Previous behaviour auto-popped the prompt-manager panel + favorites
    // popover on every star — but the user reported this was disruptive
    // ("点击收藏后让收藏夹不自动弹出来干扰人 后台静默更新收藏内容即可").
    // Now: refresh only.  If the popover is already open the new entry
    // shows up; otherwise it lands silently and the user sees it next
    // time they open the popover.
    const onStarChangedSilent = () => {
      void favoritesSidebar.refresh();
    };
    const unsubscribeStarAdded = eventBus.on('starred:added', onStarChangedSilent);
    const unsubscribeStarRemoved = eventBus.on('starred:removed', onStarChangedSilent);
    ...
    backupBtn.addEventListener('click', async () => {
      try {
        ...
        const folderData = (await folderStorage.loadData('cvFolderData')) || {
          folders: [],
          folderContents: {},
        };
