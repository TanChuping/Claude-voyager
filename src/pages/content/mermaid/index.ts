...
export const loadMermaid = async (): Promise<typeof mermaidInstance> => {
  ...
  try {
    ...
    console.error('[Claude-Voyager] Failed to load Mermaid library:', error);
    return null;
  }
};
...
const renderMermaid = async (codeBlock: HTMLElement, code: string) => {
  ...
  try {
    ...
    codeBlock.dataset.mermaidProcessing = 'false';
    console.log('[Claude-Voyager] Mermaid diagram rendered:', uniqueId);
  } catch {
    codeBlock.dataset.mermaidProcessing = 'false';
...
export const startMermaid = () => {
  ...
  chrome.storage?.sync?.get({ cvMermaidEnabled: true }, (result) => {
    ...
    if (mermaidEnabled) {
      ...
      console.log('[Claude-Voyager] Mermaid rendering is disabled');
    }
  });
  ...
  chrome.storage?.onChanged?.addListener((changes, areaName) => {
    if (areaName === 'sync' && changes.cvMermaidEnabled) {
      ...
      if (mermaidEnabled) {
        ...
        console.log('[Claude-Voyager] Mermaid rendering disabled');
      }
    }
  });
};
...
const initializeMermaid = async () => {
  ...
  if (!loaded) {
    console.warn('[Claude-Voyager] Mermaid library failed to load, diagrams will show as code');
    return;
  }
  ...
  console.log('[Claude-Voyager] Mermaid integration started');
};
