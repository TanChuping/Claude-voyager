...
export interface FavoritesSidebarArgs {
  ...
  /**
   * Same-conversation jump.  We pass the *full* message instead of just
   * `turnId` so the destination can fall back to a content-based match
   * when the stored turnId no longer resolves to a live node (Claude's
   * `data-test-render-count` increments across reloads, which used to
   * silently break "click → jump" for any star whose render-count had
   * shifted since it was created).
   */
  onJumpInPage: (message: StarredMessage) => void;
