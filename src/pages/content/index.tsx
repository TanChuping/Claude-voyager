...
import { startTimelineV2 } from './timelineV2/manager';
...
// Hosts we consider "the primary chat site" — Claude in this fork.  Kept
// in a Set + retained the legacy "CHATGPT_HOSTS" identifier so we don't
// have to rename every call site; the manifest's content_scripts match
// pattern (`https://claude.ai/*`) is what actually gates injection.
const CHATGPT_HOSTS = new Set(['claude.ai']);
...
function isChatGPTSite(): boolean {
  // Name kept for backward-compat — really means "is this the primary chat
  // site we ship features for".  For Claude-Voyager that's claude.ai (and
  // any nested subdomain, e.g. mobile.claude.ai if it ever exists).
...
async function runFeatureStep<T>(
  ...
  try {
    ...
    console.error(`[Claude-Voyager] ${featureName} failed to start:`, error);
    return null;
  } finally {
...
async function isCustomWebsite(): Promise<boolean> {
  try {
    ...
    console.error('[Claude-Voyager] Error checking custom websites:', error);
    return false;
  }
}
...
async function startChatGPTFeatures(): Promise<void> {
  await runFeatureStep('Timeline', () => startTimelineV2(), HEAVY_FEATURE_INIT_DELAY);
...
async function initializeFeatures(): Promise<void> {
  ...
  try {
    ...
    console.error('[Claude-Voyager] Initialization error:', error);
  }
}
...
(function () {
  try {
    ...
    const isSupportedSite =
      CHATGPT_HOSTS.has(hostname) || hostname === 'claude.ai' || hostname.endsWith('.claude.ai');

    if (isSupportedSite) {
      initKaTeXConfig();
      initI18n().catch((error) => console.error('[Claude-Voyager] i18n init error:', error));
    }
    ...
    window.addEventListener('beforeunload', () => {
      try {
        ...
        if (isExtensionContextInvalidatedError(error)) return;
        console.error('[Claude-Voyager] Cleanup error:', error);
      }
    });
  } catch (error) {
    if (isExtensionContextInvalidatedError(error)) return;
    console.error('[Claude-Voyager] Fatal initialization error:', error);
  }
})();
