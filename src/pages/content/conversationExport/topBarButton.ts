...
function currentConvIdFromUrl(): string | null {
  // Claude uses /chat/<uuid>; older ChatGPT used /c/<uuid>. Accept both.
...
function injectIfNeeded(): void {
  // Anchor on Claude's REAL Share button in the top-right (testid
  // `wiggle-controls-actions-share`).  Legacy fallbacks for ChatGPT +
  // older Claude builds.
  const share =
    document.querySelector<HTMLElement>('[data-testid="wiggle-controls-actions-share"]') ||
    document.querySelector<HTMLElement>('[data-testid="share-chat-button"]') ||
    document.querySelector<HTMLElement>('[data-testid="chat-menu-trigger"]');
  if (!share) return;
  ...
  btn.addEventListener('click', (event) => {
    ...
    if (!convId) {
      console.warn('[Claude-Voyager] export: no conversation ID in URL');
      return;
    }
