...
export class TimelineManager {
  ...
  async init(): Promise<void> {
    ...
    if (!this.cachePrimerInstalled) {
      ...
      try {
        ...
        console.warn('[Claude-Voyager] cache primer install failed', err);
      }
    }
    this.loadTextPins();
    ...
    try {
      ...
      if (g.chrome?.storage?.sync || g.browser?.storage?.sync) {
        ...
        const saved = localStorage.getItem('claudeTimelineScrollMode');
  ...
  private async findCriticalElements(): Promise<boolean> {
    ...
    let autoDetected = '';
    try {
      userOverride = localStorage.getItem('claudeTimelineUserTurnSelector') || '';
      autoDetected = localStorage.getItem('claudeTimelineUserTurnSelectorAuto') || '';
    } catch {}
    const defaultCandidates = [
      // Claude.ai user message bubble — top priority on claude.ai.
      // Each `<div data-testid="user-message">` is the inner text node;
      // we walk up to `[data-user-message-bubble="true"]` or its
      // `[data-test-render-count]` ancestor as the stable click target
      // when the inner bubble re-renders.
      '[data-testid="user-message"]',
      // Claude.ai render-wrapper that contains the user bubble.  Some
      // Claude layouts wrap user + assistant pairs in
      // `<div data-test-render-count>` — keep as a broader fallback.
      'div[data-test-render-count] div[data-user-message-bubble="true"]',
      // ChatGPT outer wrapper (kept for ChatGPT fallback / dev mode):
      // outer <section data-testid="conversation-turn-N" data-turn="user">
      // — ChatGPT keeps this wrapper in the DOM with stable offsetTop +
      // data-turn-id UUID even when virtualising the inner body.
    ...
    if (!firstTurn) {
      ...
      if (!userOverride && matchedSelector) {
        try {
          localStorage.setItem('claudeTimelineUserTurnSelectorAuto', matchedSelector);
        } catch {}
      }
      ...
      if (userOverride && matchedSelector && matchedSelector !== userOverride) {
        try {
          localStorage.removeItem('claudeTimelineUserTurnSelector');
        } catch {}
      }
    }
  ...
  private getConfiguredUserTurnSelector(): string {
    try {
      const user = localStorage.getItem('claudeTimelineUserTurnSelector');
      if (user && typeof user === 'string') return user;
      const auto = localStorage.getItem('claudeTimelineUserTurnSelectorAuto');
  ...
  private easeInOutQuad(t: number, b: number, c: number, d: number): number {
    ...
    const spring = (() => {
      try {
        return localStorage.getItem('claudeTimelineSpring') || 'ios';
      } catch {
        return 'ios';
      }
    })();
  ...
  private getFlowDurationMs(): number {
    try {
      const d = parseInt(localStorage.getItem('claudeTimelineFlowDurationMs') || '650', 10);
