...
function isChatGPTConversationRoute(pathname = location.pathname): boolean {
  // Claude.ai conversation routes are `/chat/<uuid>` (also handle `/chats/`).
  // Legacy ChatGPT routes were `/c/<uuid>` and are kept for back-compat in
  // case this code ever runs in mixed mode.
  return /(?:^|\/)(chat|chats|c)(\/|$)/.test(pathname);
}
...
function cleanup(): void {
  ...
  activeObservers.forEach((observer) => {
    try {
      ...
      console.error('[Claude-Voyager] Failed to disconnect observer during cleanup:', e);
    }
  });
  activeObservers = [];
  ...
  cleanupHandlers.forEach((handler) => {
    try {
      ...
      console.error('[Claude-Voyager] Failed to run cleanup handler:', e);
    }
  });
  cleanupHandlers = [];
