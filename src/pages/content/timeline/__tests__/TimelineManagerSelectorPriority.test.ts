...
describe('TimelineManager selector priority compatibility', () => {
  ...
  it('prefers built-in selectors over stale auto-detected selector cache', async () => {
    ...
    localStorage.setItem('claudeTimelineUserTurnSelectorAuto', '.stale-selector-target');
    ...
    expect(localStorage.getItem('claudeTimelineUserTurnSelectorAuto')).toBe(
  ...
  it('keeps explicit user override as highest priority', async () => {
    ...
    document.body.appendChild(main);
    localStorage.setItem('claudeTimelineUserTurnSelector', '.custom-user-turn');
