...
export class ConversationCaptureService {
  ...
  install(): void {
    ...
    // and we filter by our magic `__cvType` field.
  ...
  private drainSessionBuffer(): void {
    try {
      ...
      for (let i = 0; i < sessionStorage.length; i++) {
        ...
        if (k && k.startsWith('cv-cap-')) keys.push(k);
      }
      for (const k of keys) {
  ...
  ingest(convId: string, raw: unknown): CaptureEntry | null {
    ...
    // Claude payloads have `chat_messages` (array) instead of ChatGPT's
    // `mapping` (DAG).  We tolerate both shapes for forward-compat.
    const looksClaude =
      api && typeof api === 'object' && Array.isArray((api as ApiConversation).chat_messages);
    if (!looksClaude) return null;
    let linear: LinearConversation;
    try {
      linear = walkMapping(api);
    } catch (err) {
      console.warn('[Claude-Voyager] conversation parser failed', err);
      return null;
    }
    ...
    for (const cb of this.listeners) {
      try {
        ...
        console.warn('[Claude-Voyager] capture listener threw', err);
      }
    }
    return entry;
  }
  ...
  private handleMessage = (event: MessageEvent): void => {
    ...
    // legitimate hook message. We rely on the magic `__cvType` field as
    ...
    const data = event.data as
      | { __cvType?: string; payload?: { convId?: string; data?: unknown } }
      | undefined;
    if (!data || data.__cvType !== 'cv-conv-captured' || !data.payload) return;
