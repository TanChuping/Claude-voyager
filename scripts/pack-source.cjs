 ...
 * claude-voyager-<version>-source.zip containing the full repo MINUS:
 *   - node_modules        (reviewers run `bun install` themselves)
 *   - dist_chrome / dist_firefox / dist_edge / dist_safari (build output)
 *   - .git                (history irrelevant for review)
 *   - .tmp_*              (local probe artifacts; may contain real chat data)
 *   - .extension-key.pem  (private RSA signing key — NEVER ship)
 *   - claude-voyager-*.zip / .xpi (already-built artefacts)
 *   - .vscode, .idea      (editor state)
 *   - 公式交接.md          (personal handover doc, gitignored)
 *
 * Usage:
...
function shouldSkip(relPath, name) {
  ...
  if (name === 'claude-voyager-key-export.json') return true;
  // Already-built artefacts at the repo root.
  if (/^claude-voyager-.*\.(zip|xpi|crx)$/.test(name)) return true;
...
(async () => {
  ...
  console.log(`[pack-source] ✓ ${path.basename(zipPath)} (${fileCount} files, ${sizeMb} MB)`);
